...
// ===========================================================================
// The closed one-file form (ns5_38). Additive: everything above stays, because
// the 18 emitted files of `l4/ontology/` still use it.
//
// Two differences from the grammar above, both to make the document short enough to read end to end:
// (1) `details` is a map of GROUPS — `identification` (the fields that are also columns of the index
//     table), `base` (common to every subtype), `<subtype>` (exactly one per record), `general` (the
//     organization) and `<moduleId>` (the module namespace). The base is declared ONCE instead of being
//     repeated in each of the 13 subtypes.
// (2) Defaults are omitted, not written: `required: false`, `nullable: false`, `title` equal to the id,
//     `origin` (on the platform everything is level 1) and `layer` — which the group already gives:
//     `identification` is a column, every other group is document.
// ===========================================================================
